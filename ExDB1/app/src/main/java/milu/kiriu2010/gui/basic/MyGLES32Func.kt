    ...
    companion object {
        ...
        fun createProgram(svhandle: Int, sfhandle: Int, attrStrArray: Array<String> = arrayOf()): Int {
            val programHandle = GLES32.glCreateProgram().also {
                ...
                MyGLES32Func.printShaderInfoLog(svhandle,"vertex shader")

                // フラグメントシェーダをプログラムに追加
                GLES32.glAttachShader(it,sfhandle)
                MyGLES32Func.printShaderInfoLog(sfhandle,"fragment shader")
                ...
                // リンク失敗
                if (linkStatus[0] == 0) {
                    MyGLES32Func.printProgramInfoLog(it)
        ...
        // OpenGLのエラー状態を出力2
        // -------------------------------------
        fun checkGlError2(str: String, shader: ES32MgShader, model: MgModelAbs ) {
            var error = GLES32.glGetError()
            while ( error != GLES32.GL_NO_ERROR ) {
        ...
        fun createTexture(id: Int, textures: IntArray, bmp: Bitmap, size: Int = -1, wrapParam: Int = GLES32.GL_REPEAT) {
            ...
            MyGLES32Func.checkGlError("glBindTexture")
