...
// ------------------------------------------
class ES32Simple01Shader: ES32MgShader() {
    // 頂点シェーダ
    private val scv =
            """#version 320 es
    ...
    fun draw(vao: ES32VAOAbs,
        ...
        MyGLES32Func.checkGlError2("UseProgram",this,model)

        // VAOをバインド
        GLES32.glBindVertexArray(vao.hVAO[0])
        MyGLES32Func.checkGlError2("BindVertexArray",this,model)

        // uniform(モデル×ビュー×プロジェクション)
        GLES32.glUniformMatrix4fv(hUNI[0],1,false,u_matMVP,0)
        MyGLES32Func.checkGlError2("u_matMVP",this,model)
        ...
        when (mode) {
            ...
            GLES32.GL_LINES -> {
                ...
                MyGLES32Func.checkGlError2("glDrawArrays:GL_LINES",this,model)
            }
            // 線を描画
            GLES32.GL_LINE_STRIP -> {
                val cnt = model.datPos.size/3
                GLES32.glDrawArrays(GLES32.GL_LINE_STRIP,0,cnt)
                MyGLES32Func.checkGlError2("glDrawArrays:GL_LINE_STRIP",this,model)
            }
            // 線を描画
            GLES32.GL_LINE_LOOP -> {
                val cnt = model.datPos.size/3
                GLES32.glDrawArrays(GLES32.GL_LINE_LOOP,0,cnt)
                MyGLES32Func.checkGlError2("glDrawArrays:GL_LINE_LOOP",this,model)
            }
        }
