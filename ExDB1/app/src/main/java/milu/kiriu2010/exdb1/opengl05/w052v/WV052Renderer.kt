package milu.kiriu2010.exdb1.opengl05.w052v
...
// -------------------------------------
class WV052Renderer(ctx: Context): MgRenderer(ctx) {
    // 描画オブジェクト(トーラス)
    ...
    // シェーダ(深度値格納用)
    private lateinit var shaderDepth: WV051ShaderDepth
    // シェーダ(スクリーンレンダリング用)
    private lateinit var shaderScreen: WV051ShaderScreen
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        Matrix.invertM(matI,0,matM,0)
        Matrix.multiplyMM(matMVP4L,0,matVP4L,0,matM,0)
        shaderScreen.draw(modelBoard,boBoardScreen,matM,matMVP,matI,matVPT,matMVP4L,vecLight,0,0)
    }
    ...
    override fun onSurfaceChanged(gl: GL10?, width: Int, height: Int) {
        ...
        MyGLES20Func.createFrameBuffer(2048,2048,0,bufFrame,bufDepthRender,frameTexture)
    }
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // シェーダ(深度値格納用)
        shaderDepth = WV051ShaderDepth()
        shaderDepth.loadShader()
        // シェーダ(スクリーンレンダリング用)
        shaderScreen = WV051ShaderScreen()
