...
// シャドウマッピング
class W051Renderer: GLSurfaceView.Renderer {
    // 描画オブジェクト(トーラス:デプス)
    private lateinit var drawObjTorusDepth: W051ModelTorus
    // 描画オブジェクト(トーラス:スクリーン)
    private lateinit var drawObjTorusScreen: W051ModelTorus
    // 描画オブジェクト(板ポリゴン:デプス)
    private lateinit var drawObjBoardDepth: W051ModelBoard
    // 描画オブジェクト(板ポリゴン:スクリーン)
    private lateinit var drawObjBoardScreen: W051ModelBoard
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // シェーダプログラム登録(深度値格納用)
        programHandleDepth = W051ShaderDepth().loadShader()
        // シェーダプログラム登録(スクリーンレンダリング用)
        programHandleScreen = W051ShaderScreen().loadShader()

        // モデル生成(トーラス:デプス)
        drawObjTorusDepth = W051ModelTorus()

        // モデル生成(トーラス:スクリーン)
        drawObjTorusScreen = W051ModelTorus()

        // モデル生成(板ポリゴン：デプス)
        drawObjBoardDepth = W051ModelBoard()

        // モデル生成(板ポリゴン：スクリーン)
        drawObjBoardScreen = W051ModelBoard()
