...
// シャドウマッピング
class W057Renderer: GLSurfaceView.Renderer {
    // 描画オブジェクト(トーラス)
    private lateinit var drawObjTorus: W057ModelTorus
    // 描画オブジェクト(板ポリゴン)
    private lateinit var drawObjBoard: W057ModelBoard
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        var t = 0f
        var d = k_gaussian*k_gaussian/100f
        (0..9).forEach { i ->
        ...
        when (u_gaussian) {
            ...
            1 -> {
                ...
                renderBoard(1,1)
                // テクスチャを変更
                GLES20.glBindTexture(GLES20.GL_TEXTURE_2D,bufFrame[1])
                // フレームバッファのバインドを解除
    ...
    private fun renderBoard(g: Int, h:Int ) {
        ...
        // 板ポリゴンの描画
        gaussianShader.draw(drawObjBoard,matT,0,g,u_weight,h)
    }
