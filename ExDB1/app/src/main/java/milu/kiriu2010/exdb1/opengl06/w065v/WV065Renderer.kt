...
// ----------------------------------------
class WV065Renderer(ctx: Context): MgRenderer(ctx) {
    // 描画オブジェクト(トーラス)
    ...
    // VBO(球体＋メイン)
    private lateinit var boSphereMain: ES20VBOAbs
    // VBO(トーラス＋深度)
    private lateinit var boTorusDepth: ES20VBOAbs
    // VBO(球体＋深度)
    private lateinit var boSphereDepth: ES20VBOAbs
    // VBO(板ポリゴン)
    private lateinit var boBoard: ES20VBOAbs

    // シェーダ(メイン)
    private lateinit var mainShader: WV065ShaderMain
    // シェーダ(深度値の差分レンダリング)
    private lateinit var diffShader: WV065ShaderDiff
    // シェーダ(裏面深度値レンダリング)
    private lateinit var depthShader: WV065ShaderDepth
    // シェーダ(ライトの位置を点でレンダリング)
    private lateinit var pointShader: WV065ShaderPoint
    // シェーダ(ガウシアンブラー)
    private lateinit var gaussianShader: WV065ShaderGaussian
    // シェーダ(正射影レンダリング)
    private lateinit var orthShader: WV065ShaderOrth
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        depthShader.draw(modelTorus,boTorusDepth,matM4Torus,matMVP,vecI4Eye)
        ...
        depthShader.draw(modelSphere,boSphereDepth,matM4Sphere,matMVP,vecI4Eye)
        ...
        diffShader.draw(modelTorus,boTorusDepth,matM4Torus,matMVP,matVP4TexX,vecEye,0)

        // 球体のレンダリング(深度の差分をレンダリング)
        Matrix.setIdentityM(matM4Sphere,0)
        Matrix.multiplyMM(matMVP,0,matVP4O,0,matM4Sphere,0)
        diffShader.draw(modelSphere,boSphereDepth,matM4Sphere,matMVP,matVP4TexX,vecEye,0)
        ...
        // 水平方向にブラーをかける
        gaussianShader.draw(modelBoard,boBoard,matO,0,u_weight,1,renderW.toFloat())
        ...
        // 水平方向にブラーをかける
        gaussianShader.draw(modelBoard,boBoard,matO,0,u_weight,0,renderW.toFloat())
        ...
        mainShader.draw(modelTorus,boTorusMain,matM4Torus,matMVP,matI,matVP4Tex,
        ...
        mainShader.draw(modelSphere,boSphereMain,matM4Sphere,matMVP,matI,matVP4Tex,
                vecLight2,vecCenter,vecEye,vecAmbientColor,2)
        ...
        val boPoint = ES20VBOIp()
        boPoint.makeVIBO(modelPoint)
        pointShader.draw(modelPoint,boPoint,matVP)
        ...
        orthShader.draw(modelBoard,boBoard,matMVP,0)

        //   1:深度の差分を描画
        Matrix.translateM(matM,0,2f,0f,0f)
        Matrix.multiplyMM(matMVP,0,matO,0,matM,0)
        orthShader.draw(modelBoard,boBoard,matMVP,1)

        //   3:ブラーをかけた深度値を描画
        Matrix.translateM(matM,0,2f,0f,0f)
        Matrix.multiplyMM(matMVP,0,matO,0,matM,0)
        orthShader.draw(modelBoard,boBoard,matMVP,2)
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // シェーダ(メイン)
        mainShader = WV065ShaderMain()
        mainShader.loadShader()

        // シェーダ(深度値の差分レンダリング)
        diffShader = WV065ShaderDiff()
        diffShader.loadShader()

        // シェーダ(裏面深度値レンダリング)
        depthShader = WV065ShaderDepth()
        depthShader.loadShader()

        // シェーダ(ライトの位置を点でレンダリング)
        pointShader = WV065ShaderPoint()
        pointShader.loadShader()

        // シェーダ(ガウシアンブラー)
        gaussianShader = WV065ShaderGaussian()
        gaussianShader.loadShader()

        // シェーダ(正射影レンダリング)
        orthShader = WV065ShaderOrth()
        ...
        // VBO(球体＋深度)
        boSphereMain = ES20VBOIpnc()
        boSphereMain.makeVIBO(modelSphere)
        ...
        // VBO(球体＋深度)
        boSphereDepth = ES20VBOIp()
        boSphereDepth.makeVIBO(modelSphere)
    ...
    override fun closeShader() {
        ...
        boSphereDepth.deleteVIBO()
        boBoard.deleteVIBO()

        GLES20.glDeleteTextures(textures.size,textures,0)
        GLES20.glDeleteTextures(4,frameTexture)
        GLES20.glDeleteRenderbuffers(4,bufDepthRender)
        GLES20.glDeleteFramebuffers(4,bufFrame)
    }
}