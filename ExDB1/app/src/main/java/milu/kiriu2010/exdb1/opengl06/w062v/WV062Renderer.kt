...
// -----------------------------------------------------
class WV062Renderer(ctx: Context): MgRenderer(ctx) {
    // 描画オブジェクト(トーラス)
    ...
    // VBO(板ポリゴン＋メイン)
    private lateinit var boBoardMain: ES20VBOAbs
    // VBO(板ポリゴン＋反射)
    private lateinit var boBoardMirror: ES20VBOAbs

    // シェーダ(メイン)
    private lateinit var mainShader: WV062ShaderMain
    // シェーダ(ステンシル鏡面反射)
    private lateinit var mirrorShader: WV062ShaderMirror
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        mainShader.draw(modelTorus,boTorus,matM,matVP,matI,
        ...
        Matrix.invertM(matI,0,matM,0)
        mainShader.draw(modelSphere,boSphere,matM,matVP,matI,
                vecLight,vecEye, floatArrayOf(0f,0f,0f,0f), 1)
        ...
        Matrix.invertM(matI,0,matM,0)
        mainShader.draw(modelBoard,boBoardMain,matM,matVP,matI,
                vecLight,vecEye, floatArrayOf(0f,0f,0f,0f), 0)
        ...
        // レンダリング(板ポリゴン)
        mirrorShader.draw(modelBoard,boBoardMirror,matVP4O,0,u_alpha)
    }
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // シェーダ(メイン)
        mainShader = WV062ShaderMain()
        mainShader.loadShader()

        // シェーダ(ステンシル鏡面反射)
        mirrorShader = WV062ShaderMirror()
        ...
        // VBO(板ポリゴン＋メイン)
        boBoardMain = ES20VBOIpnc()
        boBoardMain.makeVIBO(modelBoard)

        // VBO(板ポリゴン＋ミラー)
        boBoardMirror = ES20VBOIpt()
        boBoardMirror.makeVIBO(modelBoard)
    ...
    override fun closeShader() {
        boTorus.deleteVIBO()
        boSphere.deleteVIBO()
        boBoardMain.deleteVIBO()
        boBoardMirror.deleteVIBO()
