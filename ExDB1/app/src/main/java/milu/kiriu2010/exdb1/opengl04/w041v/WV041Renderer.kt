package milu.kiriu2010.exdb1.opengl04.w041v
...
// --------------------------------------------
class WV041Renderer(ctx: Context): MgRenderer(ctx) {
    ...
    // シェーダ(ブラー)
    private lateinit var shaderBlur: WV041ShaderBlur

    // VBO(球体)
    private lateinit var boSphere: ES20VBOIpnct
    // VBO(板ポリゴン)
    private lateinit var boBoard: ES20VBOIpc
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        shaderScene.draw(modelSphere,boSphere,matM,matMVP,matI,vecLight,0,1)
        ...
        Matrix.multiplyMM(matMVP,0,matVP,0,matM,0)
        Matrix.invertM(matI,0,matM,0)
        shaderScene.draw(modelSphere,boSphere,matM,matMVP,matI,vecLight,1,0)
        ...
        shaderBlur.draw(modelBoard,boBoard,matMVP,u_useBlur,0,renderW.toFloat())
    }
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // シェーダ(シーン)
        shaderScene = WV041ShaderScene()
        shaderScene.loadShader()
        // シェーダ(ブラー)
        shaderBlur = WV041ShaderBlur()
