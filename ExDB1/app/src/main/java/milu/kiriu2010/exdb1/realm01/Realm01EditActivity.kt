...
import kotlinx.android.synthetic.main.activity_realm01_edit.*
...
class ScheduleEditActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_realm01_edit)
        ...
        if (scheduleId != -1L) {
            ...
            datRmDate.setText(DateFormat.format("yyyy/MM.dd",schedule?.date))
            datRmTitle.setText(schedule?.title)
            datRmDetail.setText(schedule?.detail)
            // アイテムをクリックして遷移してきた場合は削除を可能とするため、
            // 削除ボタンを表示する
            btnRmDel.visibility = View.VISIBLE
        }
        else {
            // "追加"ボタンから遷移してきた場合は登録処理を行うので、
            // 削除ボタンは表示しない
            btnRmDel.visibility = View.INVISIBLE
        }

        // 保存ボタンをクリック
        btnRmSave.setOnClickListener {
            when (scheduleId) {
                ...
                -1L -> {
                    realm.executeTransaction {
                        ...
                        datRmDate.text.toString().toDate("yyyy/MM/dd")?.let {
                            schedule.date = it
                        }
                        schedule.title = datRmTitle.text.toString()
                        schedule.detail = datRmDetail.text.toString()
                    }
