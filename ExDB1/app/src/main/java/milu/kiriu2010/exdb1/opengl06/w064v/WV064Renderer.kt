...
// ---------------------------------------------------------------
class WV064Renderer(ctx: Context): MgRenderer(ctx) {
    // 描画オブジェクト(トーラス)
    ...
    // VBO(レイ)
    private lateinit var boRay: ES20VBOAbs

    // シェーダ(メイン)
    private lateinit var mainShader: WV064ShaderMain
    // シェーダ(レイをレンダリング)
    private lateinit var rayShader: WV064ShaderRay
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        mainShader.draw(modelTorus,boTorus,matM,matMVP,matI,
                vecLight,vecCenter,vecEye,colorRim,  u_rimCoef )

        // レンダリング(レイモデル)
        rayShader.draw(modelRay,boRay,matVP)
    }
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // シェーダ(メイン)
        mainShader = WV064ShaderMain()
        mainShader.loadShader()

        // シェーダ(レイをレンダリング)
        rayShader = WV064ShaderRay()
