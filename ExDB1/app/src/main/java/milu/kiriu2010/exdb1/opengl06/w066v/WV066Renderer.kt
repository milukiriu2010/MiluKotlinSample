...
// ---------------------------------------
class WV066Renderer(ctx: Context): MgRenderer(ctx) {
    // 描画オブジェクト(トーラス)
    ...
    // シェーダ(メイン)
    private lateinit var screenShader: WV066ShaderScreen
    // シェーダ(モザイク)
    private lateinit var mosaicShader: WV066ShaderMosaic
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        // 板ポリゴンの描画
        mosaicShader.draw(modelBoard,boBoard,matVP,0,renderW.toFloat())
    }
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // シェーダ(メイン)
        screenShader = WV066ShaderScreen()
        screenShader.loadShader()

        // シェーダ(モザイク)
        mosaicShader = WV066ShaderMosaic()
