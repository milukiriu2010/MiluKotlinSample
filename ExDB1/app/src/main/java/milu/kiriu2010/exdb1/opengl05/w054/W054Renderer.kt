...
// シャドウマッピング
class W054Renderer: GLSurfaceView.Renderer {
    // 描画オブジェクト(トーラス)
    private lateinit var drawObjTorus: W054ModelTorus
    // 描画オブジェクト(板ポリゴン)
    private lateinit var drawObjBoard: W054ModelBoard

    // モデルをレンダリングするシェーダ
    private lateinit var screenShader: W054ShaderScreen
    private var programHandleScreen = -1
    // セピア調変換用シェーダ
    private lateinit var sepiaShader: W054ShaderSepia
    private var programHandleSepia = -1
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        (0..9).forEach { i ->
            ...
            drawObjTorus.drawScreen(programHandleScreen,matMVP,matI,vecLight1,vecEye,amb.toFloatArray())
        }

        // プログラムオブジェクトの選択
        //GLES20.glUseProgram(programHandleGray)
        ...
        // 板ポリゴンの描画
        drawObjBoard.draw(programHandleSepia,matT,0,u_grayScale,u_sepiaScale)
    }
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // モデルをレンダリングするシェーダ
        screenShader = W054ShaderScreen()
        programHandleScreen = screenShader.loadShader()

        // セピア調変換用シェーダ
        sepiaShader =  W054ShaderSepia()
        programHandleSepia = sepiaShader.loadShader()

        // モデル生成(トーラス)
        drawObjTorus = W054ModelTorus()

        // モデル生成(板ポリゴン)
        drawObjBoard = W054ModelBoard()
