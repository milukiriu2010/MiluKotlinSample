...
// -------------------------------------
class W072Renderer(ctx: Context): MgRenderer(ctx) {
    // 描画オブジェクト(球体)
    private lateinit var modelSphere: Sphere01Model

    // シェーダ(点のレンダリングを行う)
    private lateinit var pointShader: W072ShaderPoint
    // シェーダ(テクスチャへの描きこみを行う)
    private lateinit var mappingShader: W072ShaderMapping
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // シェーダ(点のレンダリングを行う)
        pointShader = W072ShaderPoint()
        pointShader.loadShader()

        // シェーダ(テクスチャへの描きこみを行う)
        mappingShader = W072ShaderMapping()
