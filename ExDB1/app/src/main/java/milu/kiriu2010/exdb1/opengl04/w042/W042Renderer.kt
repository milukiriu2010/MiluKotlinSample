...
// バンプマッピング
class W042Renderer: GLSurfaceView.Renderer {
    // 描画オブジェクト(球体)
    private lateinit var drawObjSphere: W042ModelSphere
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        // ビュー×プロジェクション座標変換行列
        vecEye = xQuaternion.toVecIII(floatArrayOf(0f,0f,5f))
        ...
        // 球体をレンダリング
        drawObjSphere.activateTexture(1,textures,bmpArray[0])
        Matrix.setIdentityM(matM,0)
        Matrix.rotateM(matM,0,-t1,0f,1f,0f)
        Matrix.multiplyMM(matMVP,0,matT,0,matM,0)
        Matrix.invertM(matI,0,matM,0)
        drawObjSphere.draw(programHandle,matM,matMVP,matI,vecLight1,vecEye,0)
    }
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // シェーダプログラム登録
        programHandle = W042Shader().loadShader()
