...
// シャドウマッピング
class W053Renderer: GLSurfaceView.Renderer {
    // 描画オブジェクト(トーラス)
    private lateinit var drawObjTorus: W053ModelTorus
    // 描画オブジェクト(板ポリゴン)
    private lateinit var drawObjBoard: W053ModelBoard

    // モデルをレンダリングするシェーダ
    private lateinit var screenShader: W053ShaderScreen
    private var programHandleScreen = -1
    // グレースケール変換用シェーダ
    private lateinit var grayShader: W053ShaderGray
    private var programHandleGray = -1
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        // 板ポリゴンの描画
        drawObjBoard.draw(programHandleGray,matT,0,u_grayScale)
    }
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // グレースケール変換用シェーダ
        grayShader =  W053ShaderGray()
        programHandleGray = grayShader.loadShader()

        // モデル生成(トーラス)
        drawObjTorus = W053ModelTorus()

        // モデル生成(板ポリゴン)
        drawObjBoard = W053ModelBoard()
