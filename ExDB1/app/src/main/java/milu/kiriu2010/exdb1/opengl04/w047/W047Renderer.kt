...
class W047Renderer: GLSurfaceView.Renderer {
    // 描画オブジェクト(立方体)
    private lateinit var drawObjCube: W047ModelCube
    // 描画オブジェクト(球体)
    private lateinit var drawObjSphere: W047ModelSphere
    // 描画オブジェクト(トーラス)
    private lateinit var drawObjTorus: W047ModelTorus
    ...
    // 点光源の位置
    private val vecLight1 = floatArrayOf(-1f,1f,1f)
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        drawObjCube.draw(programHandleB,matM,matMVP,vecEye,0,0)
        ...
        Matrix.multiplyMM(matMVP,0,matT,0,matM,0)
        drawObjSphere.draw(programHandleB,matM,matMVP,vecEye,0,1)
        ...
        targetArray.forEach {
            ...
            drawObjTorus.draw(programHandleA,matM,matMVP,vecEye,0,1)
        }
    }
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // シェーダプログラム登録
        programHandleA = W047AShader().loadShader()
        // シェーダプログラム登録
        programHandleB = W047BShader().loadShader()

        // キューブマップを生成
        generateCubeMap()

        // モデル生成(立方体)
        drawObjCube = W047ModelCube()

        // モデル生成(球体)
        drawObjSphere = W047ModelSphere()

        // モデル生成(トーラス)
        drawObjTorus = W047ModelTorus()
    ...
    // フレームバッファへの６方向レンダリング
    private fun render2FrameBuffer() {
        targetArray.forEachIndexed { id, target ->
            // フレームバッファにテクスチャを関連付ける
            GLES20.glFramebufferTexture2D(GLES20.GL_FRAMEBUFFER,GLES20.GL_COLOR_ATTACHMENT0,target,bufFrame[0],0)
            ...
            Matrix.scaleM(matM,0,100f,100f,100f)
            Matrix.multiplyMM(matMVP,0,matT,0,matM,0)
            drawObjCube.draw(programHandleB,matM,matMVP, floatArrayOf(0f,0f,0f),0,0)
