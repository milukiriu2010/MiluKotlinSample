...
// --------------------------------------------
class WV058Renderer(ctx: Context): MgRenderer(ctx) {
    // 描画オブジェクト(トーラス)
    ...
    // シェーダ(反射光)
    private lateinit var specularShader: WV058ShaderSpecular
    // シェーダ(正射影で板ポリゴンをgaussianフィルタでレンダリング)
    private lateinit var gaussianShader: WV058ShaderGaussian
    // シェーダ(正射影でレンダリング結果を合成する)
    private lateinit var finalShader: WV058ShaderFinal
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        (0..9).forEach { i ->
            ...
            specularShader.draw(modelTorus,boTorus,matMVP,matI,vecLight,vecEye)
        }
        ...
        // 板ポリゴンのレンダリング
        finalShader.draw(modelBoard,boBoard,matVP4O,0,1,u_glare)
    }
    ...
    private fun renderBoard(g: Int, h:Int ) {
        ...
        // 板ポリゴンの描画
        gaussianShader.draw(modelBoard,boBoard,matVP4O,0,g,u_weight,h,renderW.toFloat())
    }
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // シェーダ(反射光)
        specularShader = WV058ShaderSpecular()
        specularShader.loadShader()

        // シェーダ(正射影で板ポリゴンをgaussianフィルタでレンダリング)
        gaussianShader = WV058ShaderGaussian()
        gaussianShader.loadShader()

        // シェーダ(正射影でレンダリング結果を合成する)
        finalShader = WV058ShaderFinal()
