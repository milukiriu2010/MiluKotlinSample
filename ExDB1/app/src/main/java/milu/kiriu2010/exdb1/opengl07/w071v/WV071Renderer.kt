...
// -------------------------------------
class WV071Renderer(ctx: Context): MgRenderer(ctx) {
    // 描画オブジェクト(球体)
    ...
    // シェーダ(点のレンダリングを行う)
    private lateinit var pointShader: WV071ShaderPoint
    // シェーダ(テクスチャへの描きこみを行う)
    private lateinit var mappingShader: WV071ShaderMapping
    ...
    override fun onSurfaceChanged(gl: GL10?, width: Int, height: Int) {
        ...
        MyGLES20Func.createFrameBuffer(16,16,0,bufFrame,bufDepthRender,frameTexture)
    }
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // シェーダ(点のレンダリングを行う)
        pointShader = WV071ShaderPoint()
        pointShader.loadShader()

        // シェーダ(テクスチャへの描きこみを行う)
        mappingShader = WV071ShaderMapping()
