package milu.kiriu2010.exdb1.opengl06.w069
...
// -------------------------------------------
class W069Renderer(ctx: Context): MgRenderer(ctx) {
    ...
    // シェーダ(スクリーンレンダリング用)
    private lateinit var shaderScreen: W069ShaderScreen
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        (0..9).forEach { i ->
            ...
            shaderDepth.draw(modelTorus,matMVP4L)
        }
        ...
        Matrix.multiplyMM(matMVP4L,0,matVP4L,0,matM,0)
        shaderDepth.draw(modelBoard,matMVP4L)
        ...
        (0..9).forEach { i ->
            ...
            Matrix.multiplyMM(matMVP4L,0,matVP4L,0,matM,0)
            shaderScreen.draw(modelTorus,matM,matMVP,matI,matVPT,matMVP4L,vecLight,0)
        }
        ...
        Matrix.invertM(matI,0,matM,0)
        Matrix.multiplyMM(matMVP4L,0,matVP4L,0,matM,0)
        shaderScreen.draw(modelBoard,matM,matMVP,matI,matVPT,matMVP4L,vecLight,0)
    }
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // シェーダ(深度値格納用)
        shaderDepth = W069ShaderDepth()
        shaderDepth.loadShader()
        // シェーダ(スクリーンレンダリング用)
        shaderScreen = W069ShaderScreen()
